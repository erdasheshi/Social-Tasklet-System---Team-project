...
//add information about broker's update & update the version of already existing ones
  add:  function (data) {
           ...
           var i=0;
           while( i<sync.length && JSON.parse(sync[i]).broker != data.broker) {
                   i++;
               }
           if (i == sync.length)                            // does the broker already have an entry in the array ?
           {
            sync = sync.concat(data);                      //Broker doesn't exist in the array then add a new entry
                 ...
                 console.log(JSON.parse(sync[i]).version + "para");
                 JSON.parse(sync[i]).version = temp.version;    //broker exists, therefor update the existing entry with the updated version
                 console.log(JSON.parse(sync[i]).version + "mbrapa");
           }
           },
...
//will return the current version of a broker or 0 if the broker doesn't have an entry
  read_one: function (broker) {
            var i=0;
            while(i<sync.length && JSON.parse(sync[i]).broker != broker) {
                    i++;
                }
            if (i == sync.length)
            {
             ...
             return JSON.parse(sync[i]).version ;
            }
            },
